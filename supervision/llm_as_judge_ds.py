...
# Retry helper
# -----------------------------
def _with_retries(fn, max_retries: int = 5, base_sleep: float = 1.0):
    last_err = None
...
def vote_critic_outputs(outputs, question: str, related_context: str) -> str:
    ...
    majority_items = [
        ...
        if x["fields"]["verdict"] == majority_verdict
    ]
...
def main():
    ...
    ap.add_argument("--sleep", type=float, default=1.0, help="sleep seconds between rows")
    ...
    ap.add_argument("--model_name", default="deepseek-chat")
    ...
    if not api_key:
        raise ValueError("Missing env var DEEPSEEK_API_KEY. Do NOT hardcode keys.")
    ...
    context_col = "context" if "context" in df.columns else "content" if "content" in df.columns else None
    if context_col is None:
        print("[warning] No context/content column found. RELATED_CONTEXT will be empty.")
    ...
    if context_col is not None:
        df["related_context"] = df[context_col].apply(normalize_related_context)
    else:
        df["related_context"] = ""
    ...
    if args.resume and os.path.exists(args.out_csv):
        ...
        if len(out_df) != len(df):
            raise ValueError(
                f"Resume file length mismatch: out_csv={len(out_df)}, input={len(df)}. "
                "Please remove --resume or provide a matching out_csv."
            )
