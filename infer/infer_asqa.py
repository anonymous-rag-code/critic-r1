...
# Config
# -----------------------------
num_questions = 10000  # number of sub-questions to process

# ASQA dev split for evaluation
asqa_parquet_path = "devset.parquet"
output_csv_path = "asqa_dev_inference_results.csv"
...
# -----------------------------
# Data loading: ASQA parquet
# -----------------------------
def load_questions_from_asqa_parquet_method_b(
    parquet_path: str,
    n_subq: int
) -> Tuple[List[str], List[str]]:
    """
    Flatten qa_pairs into independent sub-question samples.

    Returns:
      questions: List[str]          -> sub_question
      golden_answers: List[str]    -> ' ||| ' joined short answers
    """
    ds = load_dataset("parquet", data_files={"dev": parquet_path})["dev"]

    questions: List[str] = []
    golden_answers: List[str] = []

    for ex in ds:
        if len(questions) >= n_subq:
            break

        amb_q = safe_str(ex.get("ambiguous_question", ""))
        qa_pairs = ex.get("qa_pairs", [])
        all_wikipages = ex.get("wikipages", [])
        ...
        for qa in qa_pairs:
            ...
            sub_q = safe_str(qa.get("question", ""))
            short_answers = normalize_answer_list(qa.get("short_answers", []))
            qa_wikipage = safe_str(qa.get("wikipage", ""))
            ...
            golden_answers.append(" ||| ".join(short_answers))

    if not questions:
        raise ValueError("No valid ASQA sub-questions found.")

    return questions, golden_answers


# -----------------------------
# Resume / incremental write
# -----------------------------
...
def answer_single_question(question: str, model, tokenizer, device):
    ...
    while True:
        ...
        if "</answer>" in output_text.lower():
            print(output_text)
...
def main():
    ...
    questions, golden_answers = load_questions_from_asqa_parquet_method_b(
        asqa_parquet_path, num_questions
    )
    ...
    for i, q in enumerate(questions):
        print(f"\n=== Processing Sub-Question {i+1}/{len(questions)} ===")
