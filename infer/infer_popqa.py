...
# -----------------------------
num_questions = 10000
# PopQA test split for evaluation
popqa_csv_path = "test.tsv"

output_csv_path = "popqa_test_inference_results.csv"
...
def parse_list_like_field(x: Any) -> List[str]:
    s = safe_str(x)
    if not s:
        return []
    ...
    try:
        ...
        if isinstance(val, list):
            return [safe_str(v) for v in val if safe_str(v)]
...
def parse_possible_answers_json(s: Any) -> List[str]:
    return parse_list_like_field(s)



# -----------------------------
# Data loading: PopQA TSV
# -----------------------------
def load_questions_from_popqa_csv(
    csv_path: str, n: int
) -> Tuple[List[str], List[str]]:
    df = pd.read_csv(csv_path, sep="\t")

    required_cols = ["question", "possible_answers"]
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"PopQA TSV must contain column: {col}. Got: {list(df.columns)}")
    ...
    for i in range(min(n, len(df))):
        ...
        ans_list = parse_possible_answers_json(row.get("possible_answers", ""))
...
# -----------------------------
# Resume / incremental write helpers
# -----------------------------
def get_completed_questions(csv_path: str) -> Set[str]:
    """
    Read existing output CSV and return finished question set.
    """
...
def main():
    ...
    questions, golden_answers = load_questions_from_popqa_csv(
        popqa_csv_path, num_questions
    )
